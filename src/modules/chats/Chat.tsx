...
import { createChatAsync, getListChatAsync, toggleSetKeyword, updatechatAsync } from './slice';

const { Search } = Input;

const Chat: React.FC = () => {
  const { listchat, keyword } = useSelector((state: RootState) => state.chat);
  ...
  useEffect(() => {
    if (!isSearch) {
      ...
      dispatch(getListChatAsync(params));
    }
  ...
  const handleSubmit = async (values: any) => {
    ...
    if (userSelected) {
      await dispatch(updatechatAsync({ tag: userSelected.tag, params }))
      showAlert("success", "create CarType successfully", 3);
    } else {
      showAlert("success", "create CarType successfully", 3);
      await dispatch(createChatAsync(params));   
    }
    onSearch(keyword)
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: string, record: any) => (
        <span>{record.patterns ? record.patterns.join(", ") : ''}</span>
      ),
    },
    {
      title: t('responses'),
      ...
      width: 450,
      render: (text: string, record: any) => (
        <span>{record.responses ? record.responses.join(", ") : ''}</span>
      ),
    },    
    {
      title: t('action'),
      ...
      render: (text: string, record: ChatPayload) => (
        <div className="action-buttons-container">
          <BsReply onClick={() => handleEditUser(record)} className="icon-action-edit" />
...
return (
    ...
    <ModalComponent
      ...
      >
      <p className="text-confirm text-lg text-center mb-10">{`${t('text_confirm_del')} ${userSelected?.tag}  ${t('no')} ?`}</p>
    </ModalComponent>
		</div>
	  </div>
	</div>
